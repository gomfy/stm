...
for(k in ks) {
  cum_time = 0
  for(j in 1:num_run) {
    res <- stm(poliblog5k.docs, poliblog5k.voc, K=k, prevalence=~rating, data=poliblog5k.meta, max.em.its = 100, control=list(method="BFGS"))
...
print(reg_sum_df)
save(reg_sum_df,file="reg_sum.Rda")
