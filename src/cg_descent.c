...
double      (*value) (double *, INT, INT, double*, double*, double*, double*),  /* f = value (x, n) */
void         (*grad) (double *, double *, INT, INT, double*, double*, double*, double*), /* grad (g, x, n) */
double    (*valgrad) (double *, double *, INT, INT, double*, double*, double*, double*), /* f = valgrad (g, x, n),
...
)
{
  ...
  *dsub, *gsub, *gsubtemp, *gkeep, *tau, *vsub, *wsub ;
  ...
  Com.alpha = ZERO ;
  status = cg_evaluate ("fg", "n", &Com, K_, V_, beta_, doc_ct_, mu_, siginv_);
  f = Com.f ;
  if ( status )
  {
  ...
  for (iter = 1; iter <= maxit; iter++)
  {
    ...
    if ( Parm->QuadStep )
    {
      ...
      if ( ((t > Parm->QuadCutOff)&&(fabs(f) >= Com.SmallCost)) || QuadF )
      {
        if ( QuadF )
        {
          ...
          status = cg_evaluate ("g", "y", &Com, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
          if ( status ) goto Exit ;
          if ( Com.df > dphi0 )
          {
        ...
        else
        {
          ...
          status = cg_evaluate ("f", "y", &Com, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
          if ( status ) goto Exit ;
          ftemp = Com.f ;
    ...
    Com.f0 = f ;                          /* f0 saved as prior value */
            ...
            /* perform line search */
            status = cg_line (&Com, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
  ...
  )
{
  ...
  if ( Com->QuadOK )
  {
    status = cg_evaluate ("fg", "y", Com, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
    fb = Com->f ;
  ...
  while ( db < ZERO )
  {
    if ( !qb )
    {
      status = cg_evaluate ("f", "n", Com, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
    ...
    {
      status = cg_contract (&a, &fa, &da, &b, &fb, &db, Com, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
  ...
  if ( Com->neps > Parm->neps ) return (6) ;
    ...
    Com->alpha = b ;
    status = cg_evaluate ("g", "p", Com, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
    if ( status ) return (status) ;
    b = Com->alpha ;
    qb = FALSE ;
    if ( AWolfe ) db = Com->df ;
      ...
      double* siginv_ ) 
{
  ...
  if ( !strcmp (nan, "y") || !strcmp (nan, "p") )
  {
    if ( !strcmp (what, "f") ) /* compute function */
    {
      ...
      Com->f = Com->cg_value (xtemp, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
      Com->nf++ ;
    ...
    {
      cg_step (xtemp, x, d, alpha, n) ;
      Com->cg_grad (gtemp, xtemp, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
      Com->ng++ ;
    ...
    else                            /* compute function and gradient */
    {
      ...
      {
        Com->f = Com->cg_valgrad (gtemp, xtemp, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
      }
      else
      {
        Com->cg_grad (gtemp, xtemp, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
  ...
  else                                /* evaluate without nan checking */
  {
    if ( !strcmp (what, "fg") )     /* compute function and gradient */
    {
      if ( alpha == ZERO )        /* evaluate at x */
      {
        ...
        if ( Com->cg_valgrad != NULL )
        {
          Com->f = Com->cg_valgrad (Com->g, xtemp, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
        }
        else
        {
          Com->cg_grad (Com->g, xtemp, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
          Com->f = Com->cg_value (xtemp, K_, V_, beta_, doc_ct_, mu_, siginv_) ;
        }
      }
      else
